...
# Глобальные (не изменяющиеся) параметры игры
...
def init_sound(sound_name, volume=0.5):
    """ Функция для инициализации звука """
...
def stop_sound(sound):
    """ Функция для прерывания проигрывания звука """
    sound.stop()
...
def terminate():
    """ Функция выполняет выход из программы """
    pygame.quit()
    sys.exit()


def timer():
    """ Функция возвращает текущее время для подсчета времени игры """
    return time.time()


def print_warning(*args):
    """ Функция форматирует вывод предупреждающих сообщений в консоль """
...
def print_error(*args):
    """ Функция форматирует вывод error-сообщений в консоль """
...
def show_text(surface, text, position: (list, tuple), size=26, font=None, color=black,
    ...
    """ Функция показывает текст на экране """
    ...
    # Центрирование текста при необходимости #
    if is_centered_coordinates:
        position = (text_x - text.get_width() // 2, text_y - text.get_height() // 2)
    surface.blit(text, position)  # Отображение текста


def load_image(image_path, color_key=None, scale=(TILE_SIZE, TILE_SIZE)):
    """
    ...
    :param scale: Размер изображения
    """
    ...
    if not os.path.isfile(full_name):
        ...
        sys.exit()
    image = pygame.image.load(full_name)  # Загрузка изображения
    ...
    if scale is not None:
        assert isinstance(scale, (list, tuple)) and len(scale) == 2 and all(isinstance(el, int) for el in scale), \
            f'Incorrect scale data. Excepted Tuple[int, int], got {scale}'
